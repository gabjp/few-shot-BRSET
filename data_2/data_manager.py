...
class BRSETManager():
    def __init__(self, training_classes, test_classes, n_shots, n_ways, augment = None, batch_size = 25):
    ...
    def get_train_task(self):
        ...
        train_ids = []
        train_labels = torch.empty((0, n_ways), dtype=torch.float32)

        for clss in classes:
            objs = self.data[self.data[clss] == 1].sample(n=n_shots)
            train_ids += list(objs["image_id"])
            train_labels = torch.cat((train_labels,torch.tensor(objs[classes].to_numpy())), axis = 0) 

        if self.augment == None:
            dataset = FewShotBRSET(train_ids, train_labels)
    ...
    def get_eval_task(self):
        ...
        for clss in classes:
            objs = self.data[self.data[clss] == 1].sample(n=n_shots+1)
        ...
        dataset = FewShotBRSET(test_ids, test_labels)
        test_loader = DataLoader(dataset, batch_size=n_ways, shuffle=True)
    ...
    def get_ss_split(self, val_ratio=0.1):
        ...
        train_set = FewShotBRSET(ids_train, labels_train)
        test_set = FewShotBRSET(ids_test, labels_test)
...
if __name__ == "__main__":
    ...
    n_shots = 20
    n_ways = 3
    manager = BRSETManager(training_classes, test_classes, n_shots, n_ways, augment = None, batch_size = 1)
    ...
    train_loader, test_loader = manager.get_ss_split()
